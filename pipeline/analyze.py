...
def generate_digest(items: list[dict], date: str, stats: dict, *, source_weights: dict | None = None) -> str:
    ...
    try:
        ...
        sections_by_count = sorted(grouped.items(), key=lambda kv: len(kv[1]), reverse=True)
        scored_sections: list[tuple[float, str, list[dict]]] = []
    ...
    lines.append("1. **Trends** — Which topics are gaining momentum? What's the signal vs. noise?")
    lines.append("2. **Surprises** — What's unexpected or counter-intuitive in today's digest?")
    lines.append("3. **Connections** — How do items across different topics relate to each other?")
    lines.append("4. **Action Items** — What concrete next steps does this suggest?")
    lines.append("5. **Questions** — What important questions does this raise that aren't answered here?")
    lines.append("")
    lines.append("---")
    lines.append("_Generated by herald_")
